...
fn main() {
    ...
    assert!(
        !filepath_allows_write(&output_dir),
        "Output filepath is invalid or not writable."
    );
    ...
    let max_range_value = *max_values.iter().max().expect("Error parsing max values.") - min_value;
    ...
    for max_value in max_values.iter() {
        ...
        for arr_len in (start_length..=end_length).step_by(length_increment) {
            ...
            write!(&mut experiment_text, "{arr_len}")
